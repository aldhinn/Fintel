...
from utils.request_handlers import _SymbolsHandler, RequestHandlerFactory
...
@pytest.fixture
def fixture_api_symbols() -> tuple[MagicMock, MagicMock, MagicMock, MagicMock]:
...
def test_retrieve_symbols(fixture_api_symbols):
    ...
    mock_db_session, _, _, mock_flask_app = fixture_api_symbols
    ...
    mock_all_active_query = MagicMock(return_value=[(value,) for value in expected_active_list])
    mock_db_session.query.return_value.filter_by.return_value.all = mock_all_active_query
...
def test_non_get_and_post_methods_symbols(fixture_api_symbols):
...
def test_track_add_calls(fixture_api_symbols):
    """Track the number of add calls to the database.
    """
    # Mock database interactions.
    mock_db_session, _, mock_db_session_add_call, mock_flask_app = fixture_api_symbols
    # Create the handler instance, passing the mock db session and mock flask_application
    handler = RequestHandlerFactory.create_handler(\
        API_ENDPOINT_SYMBOLS, mock_db_session, mock_flask_app)
...
def test_exceptions_raised_during_commit(fixture_api_symbols):
    """Expected to rollback database session if exception is raised.
    """
    # Mock database interactions.
    mock_db_session, mock_db_session_rollback_call, _, mock_flask_app = fixture_api_symbols
    # Create the handler instance, passing the mock db session and mock flask_application
    handler = RequestHandlerFactory.create_handler(\
        API_ENDPOINT_SYMBOLS, mock_db_session, mock_flask_app)
...
def test_posting_with_list_with_non_string_element(fixture_api_symbols):
    """ Attempt to add an asset symbol of which value is not a string.
    """
    # Mock database interactions.
    mock_db_session, mock_db_session_rollback_call,\
        mock_db_session_add_call, mock_flask_app = fixture_api_symbols
    # Create the handler instance, passing the mock db session and mock flask_application
    handler = RequestHandlerFactory.create_handler(\
        API_ENDPOINT_SYMBOLS, mock_db_session, mock_flask_app)
...
def test_posting_with_empty_list(fixture_api_symbols):
...
def test_posting_with_non_list(fixture_api_symbols):
...
def test_id_query_exception_in_yf_fetch(fixture_api_symbols):
    ...
    mock_db_session, _, mock_db_session_add_call, mock_flask_app = fixture_api_symbols
    # Create the handler instance, passing the mock db session and mock flask_application
    handler = _SymbolsHandler(mock_db_session, mock_flask_app)
...
def test_yf_download_none(fixture_api_symbols):
